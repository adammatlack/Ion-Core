...
# Get BUILDDIR and REAL_DARKSILKD
. "${CURDIR}/tests-config.sh"

export DARKSILKCLI=${BUILDDIR}/qa/pull-tester/run-darksilk-cli
export DARKSILKD=${REAL_DARKSILKD}
...
if [ "x${ENABLE_DARKSILKD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and darksilkd must all be enabled"
fi

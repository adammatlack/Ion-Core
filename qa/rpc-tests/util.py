...
import os
import sys
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "python-darksilkrpc"))
...
from darksilkrpc.authproxy import AuthServiceProxy, JSONRPCException
from util import *
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    darksilkd and darksilk-cli must be in search path.
    """
    ...
    if not os.path.isdir(os.path.join("cache", "node0")):
        ...
        for i in range(4):
            ...
            args = [ os.getenv("DARKSILKD", "darksilkd"), "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            darksilkd_processes[i] = subprocess.Popen(args)
            subprocess.check_call([ os.getenv("DARKSILKCLI", "darksilk-cli"), "-datadir="+datadir,
...
def start_node(i, dirname, extra_args=None, rpchost=None):
    ...
    args = [ os.getenv("DARKSILKD", "darksilkd"), "-datadir="+datadir, "-keypool=1", "-discover=0", "-rest" ]

...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars DARKSILKD and DARKSILKCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

DARKSILKD=${DARKSILKD:-${1}/darksilkd}
CLI=${DARKSILKCLI:-${1}/darksilk-cli}
...
$DARKSILKD -zapwallettxes=1 $B1ARGS &
B1PID=$!
$DARKSILKD -zapwallettxes=2 $B2ARGS &
B2PID=$!
